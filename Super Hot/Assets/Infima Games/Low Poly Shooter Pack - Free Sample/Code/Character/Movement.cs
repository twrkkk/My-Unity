...
namespace InfimaGames.LowPolyShooterPack
{
    ...
    public class Movement : MovementBehaviour
    {
        ...
        [SerializeField] private float _slowDownTimeScale;
        ...
        {
            _multipliyer = _normalTimeScale / _slowDownTimeScale;
        ...
        {
            Vector2 movement = playerCharacter.GetInputMovement();
            float x = movement.x;
            float y = movement.y;
        ...
        private void SpeedDownTimeScale()
        {
            if (_isSlowDown) return;

            timeHandler.Change(_slowDownTimeScale);
            speedWalking *= _multipliyer;
            speedRunning *= _multipliyer;
            _isSlowDown = true;
        }

        private void SpeedUpTimeScale()
        {
            if (!_isSlowDown) return;

            timeHandler.Change(_normalTimeScale);
            speedWalking *= (1 / _multipliyer);
            speedRunning *= (1 / _multipliyer);
            _isSlowDown = false;
        }
    }
}