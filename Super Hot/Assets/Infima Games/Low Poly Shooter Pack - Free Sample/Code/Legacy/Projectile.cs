...
public class Projectile : MonoBehaviour {
	...
	private void OnCollisionEnter (Collision collision)
	{
        ...
        {
			if (playerType == PlayerType.Enemy) return;
			PartOfEnemysBody enemyPart;
			collision.gameObject.TryGetComponent<PartOfEnemysBody>(out enemyPart);
			if(enemyPart != null)
            {
				float mult = enemyPart._damageMultiplier;
				enemyPart._healthComponent.TakeDamage(Damage * mult, -collision.relativeVelocity);
            }
        }
		...
		if (collision.transform.tag == "Player")
        {
			...
			FindObjectOfType<LevelHandler>().DiePlayer();
        }

		if(collision.transform.tag == "Hostage")
        {
			PartOfHostage part = collision.gameObject.GetComponent<PartOfHostage>();
			part.Ragdoll.ActivateRagdoll();
			part.Ragdoll.ApplyForce(-collision.relativeVelocity * 0.01f);
			Debug.Log("Hostage was killed");
		}
